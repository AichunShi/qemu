...
init_thread_data(Threads *threads, const char *thread_name, int thread_nr)
{
	...
	CPU_ZERO(&cpuset);
	CPU_SET(2+i, &cpuset);
	ret = pthread_setaffinity_np(thread_local[i].thread.thread, sizeof(cpu_set_t), &cpuset);
	if (ret != 0)
		qemu_log("thread %d fail to set cpu affinity \n", i);
...
void threaded_workqueue_submit_request(Threads *threads, void *request)
{
    ...
    int thread_index = request_to_thread_index(req);
